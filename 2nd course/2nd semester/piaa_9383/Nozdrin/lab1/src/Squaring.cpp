...
void Squaring::baseCase2() {
  ...
  for (int i = m_size/2+1; i < m_size; i++)
    for (int j = 0; j < m_size/2; j++) // +1?
      m_current[i][j] = m_counter;
  m_squares.push(Square(width/2 + 1, 0, width/2));

  m_counter++;
  for (int i = 0; i < m_size/2; i++) // +1?
    for (int j = m_size/2+1; j < m_size; j++)
      m_current[i][j] = m_counter;
  m_squares.push(Square(0, width/2 + 1, width/2));
