...
PNGEdit::PNGEdit (const char *filename)
{
...
int PNGEdit::openImage (const char *filenameToOpen)
{
    if (isOpened)
        return 1;
    int y;
    ...
    { perror("file is not recognized as a PNGEdit"); return 2; }
...
int PNGEdit::saveImage (const char *filenameToSave) const
{
    if (!isOpened)
        return 1;
//    int x,y;
...
void PNGEdit::drawCircle(int x1, int y1,
                         int x2, int y2,
                         int lineThikness,
...
void PNGEdit::rgbaFilter(int chanelToChange, int newValue)
{
    if (!isOpened)
        return;
    int x,y;
...
void PNGEdit::splitImage(int n, int m,
                         int thikness,
...
void PNGEdit::drawSquareWithDiagonals(int x1, int y1,
                                      int sideSize,
                                      int lineThikness,
                                      png_color lineColor, bool isFilled,
                                      int lineColorAlpha,
...
void PNGEdit::info() const
{
    std::cout << "Image info:" << std::endl;
    std::cout << "\tSize:                              " << this->image->width << "x" << this->image->height << std::endl;
}

bool PNGEdit::opened() const
{
    return this->isOpened;
}

char *PNGEdit::getFileName() const
{
    return this->fileName;
}

bool PNGEdit::isOutOfBounds(const int x, const int y) const
{
    if (!isOpened)
        return false;
...
void PNGEdit::setPixel(const int x, const int y, png_color color, int colorAlpha)
{
    if (!isOpened)
        return;
...
void PNGEdit::setBigPixel(const int x, const int y, png_color color, bool isVertical, int thikness, int colorAlpha)
{
    if (!isOpened)
        return;
...
void PNGEdit::drawLine(const int x1, const int y1, const int x2, const int y2, const png_color color, const int thikness, const int colorAlpha)
{
    if (!isOpened)
        return;
    if (x1==x2 && y1==y2) {
